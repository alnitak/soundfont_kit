...
void main() {
  ...
  group('Celesta_minimal.sf3 OGG Playback and Noise Tests', () {
    ...
    test(
      'Plays Celesta_minimal.sf3 notes and checks for discontinuities or noise',
      () async {
        ...
        print(
          'Presets count: ${sf.presets.length}, samples count: ${sf.samples.length}',
        );
        for (int i = 0; i < sf.samples.length && i < 5; i++) {
          final s = sf.samples[i];
          print(
            'Sample $i: name=${s.name}, rate=${s.sampleRate}, comp=${s.compression}, channels=${s.channels}',
          );
        }
        ...
        for (final note in notes) {
          print('\n--- Playing Note $note ---');
          final matchingPresetZones = preset.zones
              .where((pz) => pz.matches(note, 100))
              .toList();
          print('Matching preset zones: ${matchingPresetZones.length}');
          for (final pz in matchingPresetZones) {
            final inst =
                (pz.instrumentID != null &&
                    pz.instrumentID! < sf.instruments.length)
                ? sf.instruments[pz.instrumentID!]
                : null;
            print(
              'PresetZone: instID=${pz.instrumentID}, instName=${inst?.name}',
            );
            if (inst != null) {
              final matchingInstZones = inst.zones
                  .where((iz) => iz.matches(note, 100))
                  .toList();
              ...
              for (final iz in matchingInstZones) {
                final sample =
                    ...
                    (iz.sampleID != null && iz.sampleID! < sf.samples.length
                        ...
                        : null);
                print(
                  '  InstZone: sample=${sample?.name}, rate=${sample?.sampleRate}, origPitch=${sample?.originalPitch}, keyRange=[${iz.keyRangeMin}, ${iz.keyRangeMax}], pan=${iz.pan}, volEnvRelease=${iz.volEnvRelease}, presetVolEnvRelease=${pz.volEnvRelease}',
                );
              }
            }
          }
          ...
          print(
            'Note $note: max frame-to-frame delta = $maxDelta at relative frame $deltaIndex',
          );
        }
