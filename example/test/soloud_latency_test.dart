...
void main() {
  ...
  group('SoLoud PCM Capture Latency Tests with tic-1.wav', () {
    ...
    test('Captures PCM mixer output and verifies tic-1.wav onset latency', () async {
      ...
      expect(File(ticPath).existsSync(), isTrue, reason: 'tic-1.wav must exist at $ticPath');
      ...
      print('Total captured frames: $totalStereoFrames (${(totalStereoFrames / sampleRate * 1000).toStringAsFixed(1)} ms)');
      print('Frames before play() call: $framesBeforePlay (${(framesBeforePlay / sampleRate * 1000).toStringAsFixed(1)} ms)');
      print('Peak amplitude detected: ${peakAmplitude.toStringAsFixed(3)}');

      expect(audibleFrameIndex, isNotNull, reason: 'The tic sound must be audible in the captured output');
      ...
      print('Onset delay after play(): $onsetFramesAfterPlay frames (${onsetLatencyMs.toStringAsFixed(2)} ms)');
    ...
    test('Captures PCM mixer output and verifies SoundFont preset note onset latency', () async {
      const sampleRate = 44100;
      const bufferSize = 8192;
      const devicePeriod = 256;
      const renderAhead = 8192;
      ...
      // Play SoundFont note
      final voice = await player.playPreset(sf.presets.first, key: 60, velocity: 100);
      ...
      expect(audibleFrameIndex, isNotNull, reason: 'SoundFont note must be audible in the output');

      final onsetFramesAfterPlay = audibleFrameIndex! - framesBeforePlay;
      final onsetLatencyMs = (onsetFramesAfterPlay / sampleRate) * 1000.0;

      print('SoundFont Note onset delay: $onsetFramesAfterPlay frames (${onsetLatencyMs.toStringAsFixed(2)} ms)');

      await player.dispose();
    });

    test('Captures PCM mixer output for preloaded SoundFont preset and rapid note triggers', () async {
      const sampleRate = 44100;
      const bufferSize = 8192;
      const devicePeriod = 256;
      const renderAhead = 8192;
      ...
      final v1 = await player.playPreset(sf.presets.first, key: 60, velocity: 100);
      expect(v1.handles, isNotEmpty);

      // Play note 64 after 50 ms
      await Future.delayed(const Duration(milliseconds: 50));
      final framesBeforeNote2 = capturedFloats.length ~/ 2;
      final v2 = await player.playPreset(sf.presets.first, key: 64, velocity: 100);
      ...
      if (onset1 != null) {
        ...
        print('Note 1 (C4) onset delay: ${onset1 - framesBeforeNote1} frames (${delay1.toStringAsFixed(2)} ms)');
      }
      if (onset2 != null) {
        final delay2 = ((onset2 - framesBeforeNote2) / sampleRate) * 1000.0;
        print('Note 2 (E4) onset delay: ${onset2 - framesBeforeNote2} frames (${delay2.toStringAsFixed(2)} ms)');
      }
