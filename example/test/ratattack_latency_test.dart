...
void main() {
  ...
  group('RatAttack.sf2 Real Note Playback Latency Tests', () {
    ...
    test(
      'Continuously plays notes on RatAttack.sf2 with bufferSize 8192 and measures onset timing',
      () async {
        ...
        for (final note in notesToPlay) {
          ...
          for (final pz in matchingPresetZones) {
            final inst =
                ...
                : null;
            print(
              'PresetZone: instID=${pz.instrumentID}, instName=${inst?.name}, sampleID=${pz.sampleID}',
            );
            if (inst != null) {
              ...
              for (final iz in matchingInstZones) {
                print(
                  '  InstZone: sampleID=${iz.sampleID}, sampleRef=${iz.sampleRef?.name}',
                );
              }
            }
          }
          ...
          if (onsetFrame != null) {
            ...
            noteResults.add(
              'Note $note: onset after $delayFrames frames (${delayMs.toStringAsFixed(2)} ms), peak=${peak.toStringAsFixed(2)}',
            );
          } else {
            noteResults.add(
              'Note $note: NO ONSET DETECTED (peak=${peak.toStringAsFixed(2)})',
            );
          }
        }
