/// Installs, updates, or checks the soundfont_kit agent skills.
///
/// Running `dart run soundfont_kit:skills` is itself the consent to write
...
// The bundled agent skills teaching correct soundfont_kit usage each live at
...
Future<void> main(List<String> args) async {
  if (args.contains('-h') || args.contains('--help')) {
    stdout.writeln(
      'Usage: dart run soundfont_kit:skills [--check]\n'
      '\n'
      'Installs or updates the soundfont_kit agent skills (correct-usage '
...
String describeSkillPlan(SkillInstallPlan plan) {
  switch (plan.action) {
    case SkillInstallAction.sourceMissing:
      return 'Could not locate the bundled soundfont_kit skills. This '
          'package version may predate them, or it is a git dependency '
          'published without them.';
    case SkillInstallAction.upToDate:
      return 'The soundfont_kit agent skills are up to date '
          '(${plan.skillNames.length} installed).';
    case SkillInstallAction.install:
      return '${plan.installCount} of the soundfont_kit agent skills '
          '(${plan.skillNames.length} total) are not installed. Install them '
          'with: dart run soundfont_kit:skills';
    case SkillInstallAction.update:
      return '${plan.updateCount} soundfont_kit agent skill(s) have a newer '
          'version available. Update with: dart run soundfont_kit:skills';
  }
}
...
Future<String> installSkills({Directory? projectRoot, Uri? skillsRoot}) async {
  ...
  final bundled = rootUri == null
      ...
      : _bundledSkills(rootUri);
  if (bundled.isEmpty) {
    return 'Could not locate the bundled soundfont_kit skills; skipped '
        'skill install.';
  }
  ...
  return 'Installed $count soundfont_kit agent skill${count == 1 ? '' : 's'} '
      'into ${homes.join(', ')}.';
}
...
Future<Uri?> _resolveBundledSkillsRoot() async {
  final lib = await Isolate.resolvePackageUri(
    Uri.parse('package:soundfont_kit/soundfont_kit.dart'),
  );
